...
export class ContactMapper {
  ...
  toResponseDto(document: ContactDocument): ContactResponseDto {
    ...
    try {
      ...
      this.logger.error(
        `Error mapping contact document to DTO: ${error.message}`,
        error.stack,
      );
  ...
  toCreateData(dto: CreateContactDto): Partial<ContactDocument> {
    try {
      ...
      this.logger.error(
        `Error mapping create contact DTO: ${error.message}`,
        error.stack,
      );
  ...
  toUpdateData(dto: UpdateContactDto): Partial<ContactDocument> {
    try {
      ...
      this.logger.error(
        `Error mapping update contact DTO: ${error.message}`,
        error.stack,
      );
  ...
  private mapAddressToResponseDto(address: Address): AddressResponseDto {
    ...
    try {
      ...
      this.logger.error(
        `Error mapping address to response DTO: ${error.message}`,
        error.stack,
      );
  ...
  private mapAddressDtoToMongo(addressDto: AddressDto): Address {
    ...
    try {
      ...
      this.logger.error(
        `Error mapping address DTO to MongoDB: ${error.message}`,
        error.stack,
      );
